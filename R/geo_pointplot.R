...
geo_pointplot <- function(pointdf, region = FALSE, type = FALSE, map = "OpenStreet", popup = FALSE, na.rm = FALSE) {
    if (na.rm) {
    ...
    if (type == TRUE & popup == FALSE) {
        ...
        m <- leaflet::addCircleMarkers(m, lng = ~lng, lat = ~lat, label = ~label, color = ~pal(type))
