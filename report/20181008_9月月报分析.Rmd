...
```{r include=FALSE}
jdnew <- read.csv("D:\\data\\sx_raw\\交调数据\\jd201809.csv")
jdlast <- read.csv("D:\\data\\sx_raw\\交调数据\\jd201808.csv")
jdprevious <- read.csv("D:\\data\\sx_raw\\交调数据\\jd201709.csv")
```
...
caculate_carsmean(jdnews,"province") %>% gg_boxplot(xangle = 90,xlabname = "省级行政区",
                                                    ylabname="月平均日机动车交通量")
ggsave(filename = "D:\\交大云同步\\实习\\06_月度分析报告\\9月分析\\绘图\\省级机动车.jpg",dpi=600)
```
