---
title: "20181204_11鏈堟湀鎶ュ垎鏋<90>"
author: "鐜嬭嚧杩<9c>"
date: "2018骞<b4>12鏈<88>4鏃<a5>"
output:
...
```{r include=FALSE}
jdnew <- read.csv("D:\\data\\sx_raw\\交调数据\\jd201812.csv")
jdlast <- read.csv("D:\\data\\sx_raw\\交调数据\\jd201811.csv")
jdprevious <- read.csv("D:\\data\\sx_raw\\交调数据\\jd201712.csv")
```
...
```{r}
caculate_carsmean(jdnews,"province")[-31,] %>% gg_boxplot(xangle = 90,xlabname = "省级行政区",
                                                    ylabname="月平均日机动车交通量")
ggsave(filename = "D:\\交大云同步\\实习\\06_月度分析报告\\12月分析\\绘图\\省级机动车.jpg",dpi=600)
```
...
```{r eval=FALSE}
caculate_passcarsmean(jdnews,"province")[-31,] %>% 
  geojsonMap(mapName = "China",palette = "Reds",legendTitle = "交通量图例")
```
